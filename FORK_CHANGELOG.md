...
- **identical-text hits from different paths are collapsed before the curated deep fetch and the reserved slot** (`HEAD` — pending resolution)
  ...
  Existing mechanisms were checked first: ``prefer_curated`` reorders
  near-duplicates but never removes, and the searcher's
  ``_dedupe_rendered_hits`` keys on the SAME path and closet hits only.

  ``result_ordering.collapse_identical_text`` keys on the text with
  whitespace runs collapsed — nothing looser, so a one-line edit between two
  copies stays two documents. The first occurrence keeps the ranker's slot; a
  later higher-trust copy (memory < file < transcript < diary) takes it and
  inherits the bookkeeping — same words, the copy a reader can cite. The kept
  hit is marked on both channels: ``duplicates_collapsed`` and
  ``duplicate_of`` in ``--json``, ``⟨N identical copies collapsed⟩`` in every
  prose renderer from one producer, with one test per renderer.

  It runs BEFORE the depth decision and the reserved slot on the fast, hybrid
  and MCP-fallback routes, and on the deeper list, so the trigger,
  ``curated_first_rank`` and the truncation all see one list. The deep fetch
  now also fires when a collapse left the shallow list shorter than the
  requested limit — one extra call, only when a duplicate was actually seen;
  the common path still makes one call.

  Two existing fixtures had two hits with identical text — exactly the shape
  this collapses — and were given distinct texts so each test keeps testing
  what it names (a failed second call; one compact line per hit).

  *Tests:* 17 new (test_search_dedup: helper — keep-first, whitespace identity, curated copy takes the slot, distinct text survives, idempotent/tolerant; cmd_search — refill via one deeper call, collapse before reservation, no extra call on the common path; prose mark per renderer table/full/compact + plural + unique-hits negative; a hit that is both promoted and a collapse anchor shows BOTH markers, per renderer + JSON); 3 existing fixtures given distinct texts
...
- **the search banner states the DEPTH, never the store — three cases keyed on curated_first_rank** (`HEAD` — pending resolution)
  ...
  Two sessions read it as "this project has nothing curated on that" (#526).

  Three cases now, on one result object, keyed on the ``curated_first_rank``
  field #534 produces:

  * curated ranked inside the limit → nothing said about curation;
  * curated promoted from rank K → "1 curated hit promoted from rank K; the
    curated layer in this wing begins there — widen the pool with --limit 30";
  * none within the deep fetch → "no curated document in the top N", with NO
    rank clause because no K exists.

  The advice names the mechanism: the requested limit sizes the hybrid fusion
  candidate pool (#534), so ``--limit 30`` is "widen the pool", not "scroll
  further down". The banner never fires when there are no hits, so an error
  path cannot print it.

  Producer/consumer test written before the producer shipped:
  ``tests/test_search_banner_contract.py`` drives one result object from
  ``curated_first_rank`` to the rendered banner across all three cases and
  asserts the printed K equals the K in ``--json`` (parametrised at 14 and
  27). A banner whose number disagrees with the field is worse than none.

  The all-transcript / transcript-plus-diary split is dropped from the header;
  the per-hit ⟨transcript⟩ / ⟨diary⟩ tags carry it. ``all_transcript()``
  therefore has no production caller — kept as a tested predicate with its
  docstring corrected. Three #477 banner tests updated to the new wording, one
  now asserting the phrase "no curated document matched" is ABSENT.

  *Tests:* 8 new (test_search_banner_contract: three cases, K-equality at 14 and 27, never-claims-empty, json null, quiet); 3 #477 banner tests updated
...
- **--limit 3 reaches the curated layer — conditional deep fetch plus one reserved, marked slot** (`HEAD` — pending resolution)
  ...
  document ranks 14-27 (#526).

  **Two causes, both measured on production before the fix was written.**

  The requested limit sizes the hybrid *fusion candidate pool*, not just the
  cut: ``limit=3`` and ``limit=30`` return different top-3 drawers for the
  same query, while the bm25 route is stable across limits, and repeated
  identical calls are deterministic so this is not drift. At ``--limit 3`` the
  curated document is not ranked below the cut — it is never a candidate.
  #477's widen already fired here, but computed ``min(n*2, 40)`` = **six** at
  limit 3 while the curated layer begins at 14: it fired and landed short.

  And #477's ordering cannot lift it. That ordering is bounded to
  near-duplicates by design, and two review rounds hardened the bound. Measured
  similarity between the first curated hit and the transcripts above it:
  0.0/0.0/0.0 on one query and 0.008 on another, against a 0.35 threshold.
  #477 fixes *a transcript that quotes the card*; this is *the same topic with
  no shared 3-grams*. A re-rank is the wrong instrument.

  The fix is therefore a conditional deeper fetch to ``max(limit, 30)`` — a
  floor, not a multiple — fired only when the shallow result has no curated
  hit, and then **one reserved slot**, the last of the N, for the top curated
  hit in the ranker's own order. The promotion is marked on both channels:
  ``promoted: true`` and ``promoted_from_rank`` on the hit in ``--json``,
  ``⟨curated, promoted from rank K⟩`` in prose, and a top-level
  ``curated_first_rank`` carrying the rank in the deep fetch *before*
  reordering — after reordering it would be 1 almost every time and say nothing
  about corpus depth. A silent promotion would be #526's own error in reverse.

  Conditional because depth is not free: a limit-30 hybrid call on wing 2g
  measured 19-22 s (#533). Measured after, interleaved, n=2 per arm: base
  14.49/14.46 s vs head 14.33/15.92 s on the hybrid-fallback query, and
  0.18-0.26 vs 0.16-0.24 s on the bm25 route — no measurable regression,
  because the cost is the hybrid call itself rather than the row count.

  Acceptance on production, wing 2g, ``--limit 3``, five pre-registered
  queries, **as measured 2026-09-18 ~07:50 PDT**: four returned a curated hit
  where the base returned zero (K = 15, 27, 15, 9 promoted into the last slot);
  the fifth found no curated document within depth 30 on the bm25 route it
  takes, and reported ``curated_first_rank: null`` rather than inventing one.
  The K values are a snapshot: wing 2g is re-mined continuously and a
  re-run hours later read two of the queries at 0 hits on both arms and the
  fifth at K=8, with no code change. The durable check is the invariant
  above — a curated hit within the deep fetch is always visible at the limit,
  always marked, and ``curated_first_rank`` is the pre-reorder rank — not the
  numbers.

  The marker is produced once (``_promotion_tag``) and rendered by every prose
  renderer — ``table`` (default), ``full`` and ``compact`` — each under its own
  test. The first cut marked ``compact`` only, so the default view shipped the
  very silent promotion the slot exists to avoid (Oracle PART 44 on #534).

  *Tests:* 18 new (test_search_depth: depth floor, conditional trigger, curated_first_rank pre-reorder + null, reservation last-slot/marking/edge cases, prose mark per renderer: table, full, compact, plus a ranked-not-promoted negative); 3 #477 tests updated to the new contract
  *Files:* `mempalace/cli.py`
...
- **the daemon-unreachable message is written once, not hand-copied at 22 places** (`HEAD` — pending resolution)
  ...
  They were never 22 copies of one block, and reading them rather than
  counting them is what found that. There were **four distinct messages**:
  12 interpolating the exception, 5 with no exception at all, 2 carrying
  `/cypher returned {status}` wording, and `cmd_wings` going through
  `_read_family_fail` with a leading blank line, an `  ERROR: ` prefix and
  no diagnostics clause. Functions holding two occurrences held one of the
  first kind and one of the second — not two copies of one thing.

  ⭐ The five exception-less sites are the `data is None` case:
  `_call_daemon_rest` returns `None` on 404/401/403 and **nothing is
  raised**, so there is nothing to interpolate. Those sites name the
  failing route in their JSON instead — the only identifying information a
  404 leaves behind. The five prose sites and the five route-naming JSON
  payloads are the *same five sites*; that correspondence is design, not
  drift, and it is why the helper gained `route=` and `**extra` rather
  than just taking an exception:

      def _fail_daemon(err, want_json, *, route=None, **extra) -> None

  A helper that could not express what the call sites already said would
  make the CLI report **less** than it did — the defect this wave exists
  to remove, wearing the costume of a cleanup. Dropping a JSON field is a
  breaking change to a machine interface and is invisible to a prose diff:
  a script reading `.status` simply starts getting `null`.

  17 of the 20 sites are byte-identical afterwards on **both** channels.
  Three changes are declared rather than claimed away: the two `/cypher`
  sites rearrange their prose while their JSON stays identical; `cmd_wings`
  normalises on both channels, losing a JSON error that was a pre-rendered
  sentence embedding the daemon URL; and all twenty gain `_fail_daemon`'s
  "rejected the call" branch, so a server-side JSON-RPC error no longer
  prints "unreachable" — a fix of the #499 class, an error named for the
  layer that noticed rather than the layer that failed.

  `_daemon_tool_or_fail` keeps its own copy and is deliberately **not**
  consolidated here. It is not a duplicate: it exits `1 if unreachable
  else 2`, and its docstring says why — "the operator response differs and
  the sibling commands' single 'unreachable' message is actively wrong for
  the second". Folding it in while `_fail_daemon` exited 1 for both would
  have destroyed a deliberate distinction in six commands and shipped it
  as a cleanup. It converges in the follow-up that makes the codes agree.

  ⚠️ Nothing in the existing suite broke — not one test. Four distinct
  live messages, one with a different prefix and no diagnostics clause,
  and the suite was green across every one. The divergences were
  **untested behaviour**, which is how a 22-site hand-duplication survives:
  a prose diff says "no change" and a green suite agrees, because neither
  was watching. The new tests are JSON-first for that reason.

  *Tests:* 9 (test_cli_fail_daemon_one_message.py: helper shapes x4, per-route JSON x3, literal-count probe, cmd_wings normalisation); 3 mutations each killing a specific test
  *Files:* `mempalace/cli.py`
...
- **check-docs must not match a PR number against a commit-sha prefix** (`HEAD` — pending resolution)
  ...
  **re-triggered the very warning it documents** — the checker reads the first
  ...
  which is what produced the original 25/21 figure — 33 on record, 29 citable,
  with the derivation shown per source and the unit named (Oracle PART 25 is
  eight *sections*; two of them hold two sub-errors each).

  One of the six new records, `own-vocabulary-grep`, carries a discriminator the
  spec previously lacked: a zero produced by searching your own vocabulary is a
  retrieval problem when you know it is a miss, and a **false finding** when it is
  consumed as a fact ("the row did not render", "the test does not exist"). The
  same failed search sits in either regime, so `direction` is a property of the
  consumer rather than of the mechanism — which is precisely what retrieval keys
  on.
