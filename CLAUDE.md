...
35. **ci: postgres-backend job against a pgvector service container** (commit `da0bdbb`, 2026-05-11) — Adds a `test-postgres` job to `.github/workflows/ci.yml` running in parallel with the existing `test-linux` / `test-windows` / `test-macos` / `lint` matrix. Service container is the public `pgvector/pgvector:pg16` image with health checks; a pre-test Python step installs the `vector` extension via psycopg2 so no `psql` install on the runner is needed. Test scope is `tests/test_backends_postgres.py` only — the three `pytest.skipif`-gated tests (backend registration, drawer round-trip, L2 vector distance ordering). Full pytest already runs in `test-linux` without the postgres extra; running it again with `TEST_POSTGRES_DSN` set would double suite time for marginal coverage of three additional tests. AGE is deliberately not in the CI image — no current test exercises AGE-specific behavior, so the cheaper pgvector-only substrate is correct for now. When the knowledge-graph layer lands and adds AGE tests, the CI image swap (push our `mempalace-db:0.1` to ghcr, or build inline) is a separate concern. First-run job timing: 52 seconds total including service container startup, pip install of `.[dev,postgres]`, extension create, and the 3 smoke tests.

39. **fix(backends/chroma): catch-all metadata sanitizer in add/upsert** (commit `f499814`, 2026-05-11) — Defense-in-depth companion to the repair.py sanitizers in #1458 / commit `949cb20`. A 151,478-drawer rebuild against the canonical palace still failed at ~120K drawers with the same `ValueError: Expected metadata to be a non-empty dict, got 0 metadata attributes in add` from chromadb's `validate_metadata` — the traceback ran through `mempalace/backends/chroma.py:add → chromadb Collection.add → validate_insert_record_set → validate_metadatas → validate_metadata`. Even with sanitization at both repair-layer extract points (`_extract_drawers` + `_rebuild_one_collection`), something between the repair-layer sanitizer and chromadb's actual write call reshapes the metadatas list — likely chromadb's upsert internally splitting into add+update paths, or a deeper preprocessing step. Sanitizing at the chromadb-client chokepoint catches whatever the upstream path misses. New static helper `ChromaCollection._sanitize_metadatas_for_chromadb` coerces any None or empty-dict entry to `{"_repaired_empty_meta": True}` (same sentinel as the repair.py paths; searchable via `where={"_repaired_empty_meta": True}`). Both `add()` and `upsert()` route through it. Cost is one list comprehension per write call — negligible. **Direct-to-main commit (not via PR)** because the in-progress 151K-drawer rebuild on disks needed the fix live to make forward progress; standard PR-review cadence would have stalled the rebuild for hours. Defense-in-depth at the chokepoint is independently mergeable upstream once the rebuild completes and we have time to file it.

38. **fix(hooks): route Stop/PreCompact through palace-daemon/clients/hook.py** (commit `42ded2e`, 2026-05-11) — Replaces the bash wrapper invocation pattern in `.claude-plugin/hooks/hooks.json` with a single Python entrypoint via the daemon's hook client. Both `Stop` and `PreCompact` now invoke `python3 /home/jp/Projects/palace-daemon/clients/hook.py` with explicit `--hook stop --harness claude-code` / `--hook precompact --harness claude-code` arguments and a 30 s timeout. Description on the manifest names this the 'post-2026-05-11 split-brain fix' — the daemon's hook client now owns the routing decision (daemon vs local) instead of forking it across two bash scripts that previously made independent decisions about where to send work. Hooks weren't firing reliably under the previous shape; the staged file (`hooks.json.layer2-staged`, created 2026-05-11 06:01) just needed promotion. The previously-active `mempal-stop-hook.sh` and `mempal-precompact-hook.sh` stay in the tree — they're still tested by `tests/test_claude_plugin_hook_wrappers.py` and may be invoked by non-Claude-Code agents through different paths. Fork-only deployment config: the absolute path `/home/jp/Projects/palace-daemon/clients/hook.py` is specific to JP's homelab layout. Won't go to upstream as-is; the path shape would need to become discovery-based first (similar to how `MEMPALACE_PYTHON` + `$PLUGIN_ROOT/venv/bin/python3` + system fallback works in row 19's venv-aware resolution pattern).

37. **feat(kg): KnowledgeGraphAGE skeleton + graph bootstrap** (commit `a3ee623`, 2026-05-11) — First commit toward the Apache AGE-backed knowledge graph layer the migration plan calls for. Skeleton class `KnowledgeGraphAGE` in `mempalace/knowledge_graph_age.py` opens a Postgres connection, loads the AGE extension, sets `search_path = ag_catalog, "$user", public` for the session, and creates a graph named `mempalace_kg` in `ag_catalog.ag_graph` if absent. Idempotent bootstrap. Composes with the pgvector substrate (PR #21, commit `5e90c72`) — same `apache/age:release_PG16_1.6.0` image, same `mempalace-db` container, same psycopg2-binary dependency from the `[postgres]` extra (no new driver surface). Selectable via `MEMPALACE_KG_BACKEND=age` once the config-routing layer lands in a follow-up commit; until then, the SQLite-backed `KnowledgeGraph` stays the only live path. The AGE class mirrors the SQLite KG's public interface (constructor + close + context manager) so callers can eventually swap backends without code changes. Three pytest.skipif-gated tests in `tests/test_knowledge_graph_age.py`: `test_age_kg_instantiates`, `test_age_graph_created` (verifies `mempalace_kg` in `ag_catalog.ag_graph`), `test_age_context_manager` (verifies `_conn.closed` after `with` block). Verified against live homelab `mempalace-db`: 3/3 in 0.79s; full suite 1854 passed / 1 skipped / 106 deselected, +3 vs the 1851 baseline, zero regressions. Implementation notes: `autocommit=False` matches the SQLite KG's transaction semantics; both `LOAD 'age'` and the `SET search_path` are session-scoped so any future method taking a fresh cursor on this connection must re-run them. Future commits in this layer: `add_triple()` via Cypher MERGE/CREATE, query operations, temporal filtering (`as_of` queries), `MempalaceConfig.kg_backend` routing.

36. **docs(readme): pivot to four-layer model + Auto Dream vindication framing** (commit `55b36ca`, 2026-05-11) — Substantial README rewrite (+137/-100) reflecting three things that landed between the previous refresh (`a67be3f`, the 2026-05-10 develop sync) and now: (1) four-layer model promoted to the lede (storage / encoder / retrieval / consumption; anchored by the SME jp-realm-v0.1 finding that RLM-Qwen-7B and RLM-Llama-70B both ceiling at 46.67% recall while Familiar's deterministic pipeline hits 78.33% — model size doesn't fix invocation discipline; calibration paragraph hedges the absolute numbers and links to methodology disclosure); (2) Auto Dream framed as vindication of the verbatim-vs-derivative axis (Anthropic shipped Auto Dream in late April in two research-preview surfaces — Claude Code `/dream` consolidator and Managed Agents Dreams API with beta header `dreaming-2026-04-21`, models `claude-opus-4-7` and `claude-sonnet-4-6`, 100-session limit; the Dreams API's input-read-only / output-separate-store design ratifies the axis; replaces the pre-2026-04-21 "neither has consolidation" framing which was wrong); (3) substrate section moves from "exploring" to "in flight" with the live `mempalace-db` test container and the documented Plan-B trigger date 2026-06-08. New section "Convergence with peer systems" triangulates across Familiar, CampaignGenerator, Kent, adaptmem. Six new files in `docs/research/` shipped alongside as the citation surface (adaptmem-orthogonal-layers, two compass_artifact_*, convergent-findings-kostadis-comparison, three-mempalace-consumers, three-patterns-for-agent-memory). Tactical fixes: test count ~1500 → ~1850; sync date 2026-04-27 → 2026-05-10; fork-ahead count ~16 → ~14; drawer count ~151K → ~160K; setup commands lead with `uv sync --extra dev` matching the project CLAUDE.md; PR table refreshed against `gh pr list` showing 10 open jphein PRs. GitHub repo About description also updated to match.
